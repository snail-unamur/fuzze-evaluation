...
registry.category("web_tour.tours").add('fuzz_main_flow_tour_58', {
    ...
    steps: () => [...stepUtils.goToAppSteps('sale.sale_menu_root', markup(_t('Organize your sales activities with the <b>Sales app</b>.'))), stepUtils.openBurgerMenu(".o_breadcrumb .active:contains('Quotations')"), {
        ...
        mobile: false,
        trigger: "td[name='name'][data-tooltip*='J/i\^\~\+\*']",
        run: () => {},
    }, {
        mobile: true,
        ...
        mobile: true,
        trigger: ".o_field_widget[name=product_id] input",
        extra_trigger: ".o_field_widget[name=order_line] .oe_kanban_card:contains('J/i\^\~\+\*')",
        content: _t("Select a product, or create a new one on the fly. The product will define the default sales price (that you can change), taxes and description automatically."),
        position: "right",
        ...
        mobile: false,
        trigger: ".o_data_row:has(.o_data_cell:contains('J/i\^\~\+\*')):first .o_data_cell:first",
        content: _t('Select the generated manufacturing order'),
        position: 'bottom',
    }, {
        mobile: true,
        trigger: ".o_kanban_record .o_kanban_record_title:contains('J/i\^\~\+\*'):first",
